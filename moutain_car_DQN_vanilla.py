...
def shape_reward(position, velocity, done):
    # Récompense basée sur la position (encourager à aller vers la droite et en hauteur)
    reward = position + 0.5
...
class Net(nn.Module):
    ...
    def train_batches(self):
        ...
        with torch.no_grad():
            ...
            next_step_max_q_value = self.forward(torch.tensor(tab_next_step)).max(1)[0]
...
for iter in range(700):
    ...
    if (iter%100==0 and iter!=0):
        ...
        print(f"iter : {iter}, reward : {acc_reward/100}, eps : {net.epsilon}")
        acc_reward=0
